...
export function swAlertMensaje (_icono, _texto) {  
        swa({
            titulo: "Alertas SweetAlerts",
            text: _texto,
            icon: _icono,
            button: 'Aceptar',
            timer: '3000'
...
/*  IDIOMAS SWEET ALERT */
export function FxMostrarMensajesSAdeArrays (_icono, _IdTexto) { 
    const idiomaActCook = cookies.get("ckIdiomaActual");    
    //console.log('idiomaActCook = ', idiomaActCook);

    if( idiomaActCook === 'es'){ 
        return  swAlertMensaje(_icono, mensajesSAEspanish[_IdTexto]);
    }
    if( idiomaActCook === 'en'){ 
        return  swAlertMensaje(_icono, mensajesSAEnglish[_IdTexto]);
    }
    if( idiomaActCook === 'zh'){ 
        return  swAlertMensaje(_icono, mensajesSAChinesee[_IdTexto]);
    }
    if( idiomaActCook === 'ru'){ 
        return  swAlertMensaje(_icono, mensajesSARussian[_IdTexto]);
    }
    if( idiomaActCook === 'de'){ 
        return  swAlertMensaje(_icono, mensajesSAAleman[_IdTexto]);
    }    
    if( idiomaActCook === 'hr'){ 
        return  swAlertMensaje(_icono, mensajesSACroacia[_IdTexto]);
    }
    if( idiomaActCook === 'hi'){ 
        return  swAlertMensaje(_icono, mensajesSAIndia[_IdTexto]);
    }
    if( idiomaActCook === 'ar'){ 
        return  swAlertMensaje(_icono, mensajesSAArabe[_IdTexto]);
    }
    if( idiomaActCook === 'fr-CH'){ 
        return  swAlertMensaje(_icono, mensajesSAFrances[_IdTexto]);
    }
    if( idiomaActCook === 'it-CH'){ 
        return  swAlertMensaje(_icono, mensajesSAItaliano[_IdTexto]);
    }
    if( idiomaActCook === 'ga-IE'){ 
        return  swAlertMensaje(_icono, mensajesSAIrlandes[_IdTexto]);
    }
    if( idiomaActCook === 'pt-BR'){ 
        return  swAlertMensaje(_icono, mensajesSAPortug[_IdTexto]);
    }  
    else{
    }
